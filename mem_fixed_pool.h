...
namespace Mem
{
    ...
    FixedPool< size>::allocateChunk()
    {
        ...
        chunk->ListAll::attach( first_chunk);
        chunk->ListFree::attach( free_chunk);
        first_chunk = chunk;
        free_chunk = chunk;
    ...
    FixedPool< size>::deallocateChunk( MemImpl::Chunk< size> *chunk)
    {
        ...
        {
            first_chunk = static_cast<MemImpl::Chunk<size>*>(chunk->ListAll::next());
        }
    ...
    FixedPool< size>::allocate( size_t sz)
    {
        ...
        free_chunks--;
            ...
            free_chunk = static_cast<MemImpl::Chunk<size>*>(chunk->ListFree::next());
            chunk->ListFree::detach();
        }
        entry_count++;
    ...
    /** Output pool's statistics to a stream */
    template < size_t size> 
    void
