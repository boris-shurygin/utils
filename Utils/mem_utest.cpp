...
static bool uTestPools( UnitTest *utest_p)
{
    ...
    for ( int i = 0; i < 20000; i++)
    {
        ...
        obj->attach( prev_obj);
        prev_obj = static_cast<MyPoolObj *>(obj->next());
    }
    while ( isNotNullP( obj))
    {
        MyPoolObj *next = static_cast<MyPoolObj *>(obj->next());
        pool->destroy( obj);
        obj = next;
    }
    delete pool;
...
/**
 * Test pool-based allocator with lists
 */
...
/**
 * Measure standard allocator with lists
 */
